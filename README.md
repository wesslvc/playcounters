...
- 같은 곡이 출처마다 다른 제목으로 오면 하나로 묶습니다 — 아래 참고
- 팟캐스트·오디오북 제외
- 날짜는 한국 시간(`Asia/Seoul`) 기준 — 바꾸려면 `app/api/stats/route.js`의 `TZ`

### 제목 합치기

같은 녹음이 출처마다 다른 제목으로 들어옵니다. Spotify는
`Lose My Mind (feat. Doja Cat) [From F1® The Movie]`, YouTube는
`Lose My Mind (feat. Doja Cat)`, 공식 채널 업로드는 앞에 가수 이름이 붙습니다.
문자열 그대로 세면 한 곡이 넷으로 쪼개집니다.

그래서 `norm_track()`으로 정규화한 값으로 묶습니다. 지우는 것은 **녹음 자체를
바꾸지 않는 수식어**뿐입니다 — 피처링 표기, `(From ...)` 사운드트랙 표기,
`(Official Music Video)` 같은 표시, 리마스터·에디션 표기, 그리고 제목 앞의
`가수 - ` 접두사.

리믹스·라이브·어쿠스틱·인스트루멘탈·슬로우드·8D는 **실제로 다른 녹음이므로
합치지 않습니다.** 표시되는 제목은 그 묶음에서 가장 많이 등장한 표기입니다.
