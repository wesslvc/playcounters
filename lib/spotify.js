...
export async function searchCover(kind, artist, name) {
  ...
  const pick = (items, checkArtist) => {
    for (const hit of items) {
      // The field-qualified query already constrained the artist through
      // Spotify's own matching, which handles credit spellings we can't —
      // "MINNIE ((G)I-DLE)" against "MINNIE". Re-checking it here on top of
      // that rejects legitimate hits, so the strict pass only verifies the
      // title. The loose query constrains nothing, so it gets both checks.
      const ok = checkArtist
        ? hitMatches(kind, hit, artist, name)
        : (kind === 'artist' ? namesMatch(artist, hit.name) : namesMatch(name, hit.name));
      if (!ok) continue;
  ...
  const strictHit = pick(first, false);
  if (strictHit) return { url: strictHit, stage: 'strict' };

  const second = await searchOnce(loose, type);
  const looseHit = pick(second, true);
  if (looseHit) return { url: looseHit, stage: 'loose' };

  return {
    url: null,
    stage: first.length || second.length ? 'rejected' : 'no-candidates',
  };
}
