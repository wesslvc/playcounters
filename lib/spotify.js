 ...
 *
 * Two passes. The field-qualified query is precise but brittle: a subtitle,
 * a different edition, or a stray bracket and it matches nothing. When it
 * comes back empty the same words go out as a plain query, which is looser
 * but recovers most of what the strict form drops.
 *
...
export async function searchCover(kind, artist, name) {
  ...
  const strict = kind === 'artist'
    ? `artist:"${a}"`
    : `${type}:"${n}" artist:"${a}"`;
  const loose = kind === 'artist' ? a : `${n} ${a}`;

  for (const q of [strict, loose]) {
    const hit = await searchOnce(q, type);
    if (hit) {
      // A track's artwork lives on its album.
      const images = type === 'track' ? hit.album?.images : hit.images;
...
async function searchOnce(q, type) {
  ...
  if (res.status === 429) {
    // Spotify says how long to wait; carry it so callers can actually honour
    // it instead of retrying straight into the same wall.
  ...
  const json = await res.json();
  return json[`${type}s`]?.items?.[0] ?? null;
}
